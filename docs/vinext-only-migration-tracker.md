...
| 5 | Update smoke/dev/build/deploy scripts | root scripts target `@datamix/app`, smoke runs against port `3000`, `.dev.vars` moves to `apps/app` | Done |
...
- 2026-06-03: Started Session 5. Retargeting root scripts and smoke coverage from the split admin/API flow to the unified `@datamix/app` Vinext Worker while leaving `apps/api` untouched for Session 6.
- 2026-06-03: Completed Session 5. Root dev/build/check/typegen/deploy scripts now target `@datamix/app`; the dev wrapper starts the unified Vinext app on `127.0.0.1:3000`; the Cloudflare Vite plugin can use `DATAMIX_PERSIST_TO` for isolated smoke state; shared local runtime defaults now point at port `3000`; and the smoke harness writes temporary `apps/app/.dev.vars`, starts the unified app, and exercises the equivalent setup/login/session, collection, record, media, public API, and health coverage. Verified `npm run check`, `npm run build`, and `npm run smoke`.
- 2026-06-04: Started Session 6. Removing legacy split architecture, Hono app dependency usage, obsolete SPA shell generation, and split admin/API documentation now that the unified `@datamix/app` Worker has passed Session 5 verification.
- 2026-06-04: Completed Session 6. Removed the legacy `apps/admin` and `apps/api` workspaces and the obsolete admin SPA shell generator; updated README, architecture, local development, onboarding, deploy/runtime, app README, and create-datamix docs for one `@datamix/app` Vinext Worker on port `3000`; switched create-datamix template generation to `apps/app`; added `apps/app/.dev.vars.example`; inlined the small shadcn Tailwind helper CSS and removed the unused `shadcn` dependency so `package-lock.json` no longer retains Hono. Verified `npm run check`, `npm run build`, and `npm run smoke`.
